/*{
    "DESCRIPTION": "classic plasma five",
...
uniform float val_n2; // @expose 1 3
uniform float val_n480; // @expose 240 720
uniform float val_n640; // @expose 320 960
...
float pattern2( in vec2 p, out vec2 q, out vec2 r , in float _time)
{
    ...
    q.x = fbm( p + vec2(_time,_time),oc,l,g);
    q.y = fbm( p + vec2(5.2*_time,1.3*_time) ,oc,l,g);
...
void main() {
    ...
    vec2 q = gl_FragCoord.xy / vec2(val_n640,val_n480);
    vec2 p = -1.0 + val_n2 * q;
    vec2 qq;
    vec2 r;
    float color = pattern2(p*(1.+mouse),qq,r,time*0.01);
    
    vec3 c = vec3(color * p.x + qq.y + qq.x,qq.x + p.y * color + r.y + r.y, qq.x * r.x + qq.x+color);
    c *= 2.;
    
    gl_FragColor = vec4(c, max(c.r, max(c.b, c.g)));
}
