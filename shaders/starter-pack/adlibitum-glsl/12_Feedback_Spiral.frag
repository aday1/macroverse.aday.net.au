...
/* AdLibitum Feedback spiral — map FFT bands to bass/mid/treb in Macroverse Params */
#define t (time * speed)
...
void main(){
  ...
  float a = atan(p.y,p.x) + t*(0.4+Md) + B;
  float r = length(p);
  float spiral = abs(sin(a*5.0 - r*(12.0+Tr*10.0) + t*2.0));
  float v = exp(-r*(1.5-Amp*0.8))*spiral + kick*exp(-r*4.0);
  vec3 col = vec3(0.04,0.02,0.08) + vec3(0.9,0.4,0.95)*v*0.7 + vec3(0.3,0.8,1.)*Md*v*0.3;
