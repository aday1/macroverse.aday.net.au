...
/* AdLibitum Plasma kick — map FFT bands to bass/mid/treb in Macroverse Params */
#define t (time * speed)
...
void main(){
  ...
  float v = sin(p.x*(5.+B*8.)+t*(0.8+Md)) + sin(p.y*(6.+Tr*6.)-t) + sin(length(p)*(8.+Amp*10.)-t*2.);
  v = v*0.2 + 0.45 + kick*0.25;
  vec3 col = sheet(v);
  col = mix(col, vec3(0.9,0.5,0.1), smoothstep(0.5,1.,v)*0.45);
  col += vec3(1.,0.3,0.05)*B*0.35;
