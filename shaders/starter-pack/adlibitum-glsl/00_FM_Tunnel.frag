...
/* AdLibitum FM tunnel — map FFT bands to bass/mid/treb in Macroverse Params */
#define t (time * speed)
...
void main(){
  ...
  float zoom = 1.2 + Md*1.8 + Amp*0.5;
  vec2 q = p * zoom;
  float fb = 0.4 + B*1.2;
  float modw = sin(t*4.0 + q.x*10.0);
  float car = sin(length(q)*18.0 - t*3.0 + fb*modw + Tr*2.0);
  float rings = abs(sin(length(q)*10.0 - t*2.0 - B*4.0));
  float v = exp(-abs(car)*1.2)*0.7 + rings*0.25 + kick*0.2;
  vec3 col = vec3(0.05,0.08,0.04) + vec3(0.7,0.95,0.4)*v + vec3(0.4,0.2,0.05)*B;
