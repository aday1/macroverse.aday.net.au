...
/* AdLibitum DOS matrix — map FFT bands to bass/mid/treb in Macroverse Params */
#define t (time * speed)
...
void main(){
  ...
  float cols = 36.0 + dens*10.0;
  float cx = floor(uv.x*cols);
  float spd = 0.2 + hash(vec2(cx,0.))*0.4 + B*0.5;
  float y = fract(uv.y + t*spd);
  float glyph = step(0.4, hash(vec2(cx, floor(uv.y*50.0 - t*(4.0+Tr*6.0)))));
  float rain = exp(-y*(4.0-Amp))*glyph;
  float glitch = step(0.96 - Amp*0.05, hash(vec2(floor(uv.y*25.0), floor(t*12.0))));
  vec3 col = vec3(0.0,0.04,0.01) + vec3(0.15,0.95,0.3)*rain + vec3(0.8,1.,0.5)*glitch*0.4;
