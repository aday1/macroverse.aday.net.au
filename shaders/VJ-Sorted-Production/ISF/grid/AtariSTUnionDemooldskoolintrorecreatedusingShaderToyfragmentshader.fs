/*{
    "DESCRIPTION": "AtariSTUnionDemooldskoolintrorecreatedusingShaderToyfragmentshader",
...
// a python scipt I threw together - https://www.dropbox.com/s/gc86xyc41t07q6g/compileimage.py?dl=0
...
void main( void ) {
    ...
    vec4 bg = vec4(0.0);
    sx = mod(floor(gl_FragCoord.x / 3.0), 48.0);
    sy = mod(floor(time * 50.0 + gl_FragCoord.y / 3.0), 48.0);
	drawSpriteUnionBackground(bg, sx, sy);   

    // animate the logo horizontally & render it
    sx = floor(sin(time*3.0) * cos(time*2.0) * 280.0*0.3 + 260.0*0.5);
    sy = 4.0;
    c2 = vec4(0.0);
