/*{
    "DESCRIPTION": "RayMarching-DiamondHexWorld",
...
float map(vec3 p)
{
	...
	float scale = max(1.0, min(abs(p.y)*0.5, mouse.x));
	vec2 grid = vec2(0.692, mouse.y) * scale;
	float radius = inputColour.w * scale;
	...
	h += max(hexd, -0.005)*inputColour.x;
...
void main( void ) {
	...
	result.xyz += vec3(glow*9, glow*mouse.x, glow*inputColour.y);
	gl_FragColor = result;
}
