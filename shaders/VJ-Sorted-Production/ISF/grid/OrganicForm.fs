...
void main( void ) {
	vec2 position = mouse / 2.0;

	float px = gl_FragCoord.x / resolution.x - 0.8 +position.x;+ (sin(time) * 0.1);
    float py = gl_FragCoord.y / resolution.y - 0.8 + position.y+(cos(time) * 0.1);
    vec2 p = vec2(px, py);
