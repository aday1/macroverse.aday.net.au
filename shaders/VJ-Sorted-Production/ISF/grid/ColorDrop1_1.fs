...
vec3 getColors( vec2 position ) {
	vec3 res = vec3(cos(position.x), sin(position.y), 1. - 1.5 * cos(time));
	return res;
}
...
float getCheckerboardColor( vec2 position ) {
	float  xpos = floor(13.0 * position.x );
	float  ypos = floor( 11.0 * position.y );
	float col = mod( xpos, 3.0 );
	if( mod( ypos, 2.0 ) > 1.5 ) {
		col = cos(xpos*ypos + time * 5.);
	} else {
		col = sin(xpos*ypos + time * 7.);
