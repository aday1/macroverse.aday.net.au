/*{
    "DESCRIPTION": "Boxes-MinusCheckerboard",
...
// Dunno--checkerboard flat thing
// src: http://glslsandbox.com/e#29802.0
#ifdef GL_ES
...
vec2 obj0(in vec3 p){
    //obj deformation
    p.y=p.y+sin((pow(sqrt(p.x*p.x+p.z*p.z),78.)/max(sqrt(p.x*p.x+p.z*p.z),2.))-time*4.0)*0.5;
    //plane
