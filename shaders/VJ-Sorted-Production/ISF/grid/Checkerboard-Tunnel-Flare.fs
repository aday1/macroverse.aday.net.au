...
{
	vec2 pos = uv * 2.0 - mouse.x;
	pos.x *= (resolution.x / resolution.y);
	
	vec3 c = vec3( inputColour.z, inputColour.x, inputColour.y );
	
	for( float i = 1.0; i < ParticleCount+inputColour.w; ++i )
	{
...
void main(void)
{
    ...
    vec3 finalColor = tunnel( p , mouse.y, inputColour.w);
