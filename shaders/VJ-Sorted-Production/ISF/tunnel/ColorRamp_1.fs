...
float text(vec2 uv, vec2 info)
{
    ...
    print_pos = floor(center - vec2(STRWIDTH(15.0),STRHEIGHT(1.0))/2.0);
    col += char(ch_C,uv);
    col += char(ch_o,uv);
    col += char(ch_l,uv);
    col += char(ch_o,uv);
    ...
    print_pos = floor(center - vec2(STRWIDTH(20.0),STRHEIGHT(3.0))/2.0);
    col += print_number(info.x, uv);
    col += char(ch_spc,uv);
    print_pos = floor(center - vec2(STRWIDTH(0.0),STRHEIGHT(3.0))/2.0);
...
void _userMain()
{
    ...
    float pixel = text(duv, mouse);

    float theta = (gl_FragCoord.x / resolution.x * mouse.x + mouse.y) * 6.28318530 ;
    vec3 col = vec3(sin(theta + 2.0943951),
                    sin(theta),
