...
const vec3 boundingSize = vec3(30,20,0.8)*scale;
...
{	
	...
	letters = min(letters,D(p.xy-vec2(-0.50,0.9)));
	letters = min(letters,A(p.xy-vec2( 0.00,0.9)));
	letters = min(letters,N(p.xy-vec2( 0.50,0.9)));
	
	letters = min(letters,W(p.xy-vec2( -1.,.2)));
	letters = min(letters,H(p.xy-vec2( -.5,.2)));
	letters = min(letters,I(p.xy-vec2( 0.,.2)));
	letters = min(letters,T(p.xy-vec2( .5,.2)));
	letters = min(letters,E(p.xy-vec2( 1.,.2)));
	
	p.y += 0.8;
	letters = min(letters,T(p.xy-vec2( -.75,.2)));
	letters = min(letters,R(p.xy-vec2( -.25,.2)));
	letters = min(letters,A(p.xy-vec2( 0.25,.2)));
	letters = min(letters,P(p.xy-vec2( .75,.2)));
	p.y += 0.7;
	letters = min(letters,H(p.xy-vec2( -1.,.2)));
	letters = min(letters,O(p.xy-vec2( -.5,.2)));
	letters = min(letters,U(p.xy-vec2( 0.,.2)));
