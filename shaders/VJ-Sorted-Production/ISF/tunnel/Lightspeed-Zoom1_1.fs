...
void _userMain( void )
{
	...
	float white1 = max(0.,inputColour.y - dist * 100.0 / (kMotionBlur*speed+adist));
	...
	float nebD1 = inputColour.z/rad + 4.5*(1.0 + sin(1.1 + 3.0*A + 0.71*cos(2.0*A)));
	...
	float R2 = 1.0 * rad * (inputColour.z + sin(1.1+4.0*A + 3.2 * cos(0.7+4.0*A)*sin(1.7+0.27*(nebD2+speed*time)) + cos(3.0*6.283*position.y) ));
	...
	color.b += inputColour.w*R1 + 0.8*R2 + .1*P2*R1;
