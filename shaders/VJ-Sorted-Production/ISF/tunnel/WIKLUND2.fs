...
// https://www.youtube.com/watch?v=ZNS7IqmZBbA
#ifdef GL_ES
...
{	
	...
	p.x += .5;
	letters = min(letters,W(p.xy-vec2(-1.00,0.9)));
	letters = min(letters,I(p.xy-vec2(-0.50,0.9)));
	letters = min(letters,K(p.xy-vec2( 0.00,0.9)));
	letters = min(letters,L(p.xy-vec2( 0.50,0.9)));
	letters = min(letters,U(p.xy-vec2( 1.,0.9)));
	letters = min(letters,N(p.xy-vec2( 1.5,0.9)));
	letters = min(letters,D(p.xy-vec2( 2.,0.9)));
