/*{
    "DESCRIPTION": "DotMatrix-OrganicForm",
...
void _userMain( void ) {
	...
	float px = position.x;//gl_FragCoord.x / resolution.x - 0.5 - (sin(time) * 0.1);
    float py = position.y;//gl_FragCoord.y / resolution.y - 0.5 - (cos(time) * 0.1);
