...
float fbm11(float p)
{
	return noise11(p) * mouse.x + noise11(p * 2.0) * mouse.y + noise11(p * 5.0) * 0.125;
}
...
void main(void)
{
	...
	ro.y = cos(0.0 * time) * (time*0.011421)*sin(tim3*12.5) - 0.0;
	// camera look at
	vec3 ta = vec3(5.0, 0.0, 0.0);
	
	// camera shake intensity
	float shake = clamp(0.0 * (0.0 - length(ro.yz)), 0.0, 0.0);
	float st = mod(time, 0.0) * 0.0;
	
	// build camera matrix
	vec3 ww = normalize(ta - ro + noise31(st) * shake * 0.00);
	vec3 uu = normalize(cross(ww, normalize(vec3(12.0, 14.0, 20.0 * sin(tim3)))));
