/*{
    "DESCRIPTION": "DotMatrix-ElectricArc-27",
...
float fbm2( vec2 p ) {
    p*=3.;
    float f = 0.0;
        f += 0.25000*cnoise2( p ); p = m2*p*0.02;
        f += 0.12500*cnoise2( p ); p = m2*p*0.13;
        f += 0.06250*cnoise2( p ); p = m2*p*0.01;
        f += 0.03125*cnoise2( p ); p = m2*p*0.04;
        f += 0.01500*cnoise2( p );
    return f/0.38375;
}
