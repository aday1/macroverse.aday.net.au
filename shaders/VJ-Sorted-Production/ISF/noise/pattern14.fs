...
float srdnoise(in vec2 P, in float rot, out vec2 grad) {
  ...
  // Transform inputVal point to the skewed simplex grid
...
void main() {
	...
	float filterVal = smoothcircle( -mouse + 0.5 +uv_distorted, 0.15, 64.);
	uv_distorted = complex_mul(((uv_distorted - 0.5)*mix(2., 12., filterVal)), mousePolar) + offset;
