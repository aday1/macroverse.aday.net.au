/*{
    "DESCRIPTION": "60s-PsychedelicShaperfractaltoomuchlsd",
...
#define l 120
vec3 neato(float it){
	vec2 v = (gl_FragCoord.xy - resolution/2.0) / min(resolution.y,resolution.x) * 30.;
	...
	return vec3(ccc, cc, c); 
}

float FNb( vec2 p, float k) {
	return sin( length(p) ) * cos( k*atan(p.y,p.x));
}

vec3 experiment(in vec2 p, float k, float m, float t ) {
	...
	vec2 A = vec2(k,0);
	vec2 B = vec2(k*cos(t*m),k*sin(t*m*m));
	vec2 C = vec2(0,-k);
	...
	if(-0.1 < z && z < 0. || 0.2 < z) return vec3(1.-1./abs(z));
	return vec3(1.-z*z);
}
...
void _userMain(void){
	...
	vec2 position = ((gl_FragCoord.xy / resolution.xy ) - (mouse)) * 250. * cos(t*0.1);
	vec3 a = experiment( position, 20.+cos( t ) * 10., sin( t * 0.2 ) * 0.25, 5.*sin(t*1.1) ); 
	vec3 b = neato( t );
	gl_FragColor = vec4( (0. < a.x) ? (a - b.xyz) : b.xyz , 1.0 );
}
