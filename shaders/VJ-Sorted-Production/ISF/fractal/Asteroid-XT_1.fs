...
float heightMapif(vec3 rad,float d)
{
    ...
    float a=_customNoise3(rad*1.6)*0.885;
    float na=a;
    if (a>line) a=pow((a-line)*iline,1.8)*(1.0-line)+line;
    if (abs(d-a)<0.2) 
    {
        na+=_customNoise3(rad*8.0)*0.1;
        a=na;
        if (a>line) a=pow((a-line)*iline,1.8)*(1.0-line)+line;
    }
    if (abs(d-a)<0.02)
    {
        na+=_customNoise3(rad*32.0)*0.01;
        a=na;
        if (a>line) a=pow((a-line)*iline,1.8)*(1.0-line)+line;
    }
    if (abs(d-a)<0.01) a+=_customNoise3(rad*128.0)*0.005;
    return a;
}
...
{
    float iline=1.0/(1.0-line);
    float a=_customNoise3(rad*1.6)*0.885+_customNoise3(rad*8.0)*0.1
	   +_customNoise3(rad*32.0)*0.01+_customNoise3(rad*64.0)*0.005;
    if (a>line) a=pow((a-line)*iline,1.8)*(1.0-line)+line;
    return a;
}
...
void main()
{
    ...
    gl_FragColor.a = 1.0;
	if (fd<line) 
         gl_FragColor.xyz = d*mix(vec3(0.21,0.19,0.0),vec3(1.0,0.99,1.0),_customNoise3(rt*128.0)*0.9+_customNoise3(rt*8.0)*0.1);
    else gl_FragColor.xyz = d*mix(mix(vec3(1.0,1.0,0.9),vec3(0.8,0.79,0.7),_customNoise3(rt*128.0)*0.9+_customNoise3(rt*8.0)*0.1),vec3(1.0),pow(fd+0.5,10.0));  
