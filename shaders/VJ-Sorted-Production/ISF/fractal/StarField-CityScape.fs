/*{
    "DESCRIPTION": "StarField-CityScape",
...
float field(in vec3 p,float s) {
	...
	for (int i = 0; i < 26; ++i) {
		float mag = dot(p, p);
		p = abs(p) / mag + vec3(-.5 + mouse.y * 0.01, -.4 + mouse.x * 0.01, -1.5);
...
void main() {
	...
	p += .2 * vec3(sin(time / 16.) + mouse.x, sin(time / 14.) + mouse.y,  sin(time / 128.));
	...
	vec4 c2 = mix(.4, 1., v) * vec4(1.3 * t2 * t2 * t2 + mouse.x ,1.8  * t2 * t2 + mouse.y, t2* freqs[0], t2) * 0.2;
