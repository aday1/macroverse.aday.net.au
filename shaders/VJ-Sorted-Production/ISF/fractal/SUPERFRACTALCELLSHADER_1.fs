/*{
    "DESCRIPTION": "SUPERFRACTALCELLSHADER",
...
void main(void)
{
	...
	vec2 mousePos =(iMouse.xy/iResolution.xy-.5)*3.;
	if (iMouse.z<1.) mousePos=vec2(0.,-0.05);
	float fov=.9-max(0.,.7-iGlobalTime*.3);
	vec3 dir=normalize(vec3(uv*fov,1.));
	dir.yz*=rot(mousePos.y);
	dir.xz*=rot(mousePos.x);
