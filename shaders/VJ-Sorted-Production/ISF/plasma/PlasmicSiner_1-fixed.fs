...
void main( void ) {
	...
	float localTime = time + pow(cos(p.x*33.)*cos(p.x*33.), sin(p.y*55.)*sin(p.y*64.));
	
	p.y += sin(p.x+localTime)*0.5/p.x;
	
	float sx = 0.3 * (p.x + 0.8) * sin( 25.0 * p.x - 1. * pow(localTime, 0.09)*4.);
