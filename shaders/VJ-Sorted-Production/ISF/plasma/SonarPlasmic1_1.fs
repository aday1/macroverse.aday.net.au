...
float angle(vec2 orig){
	vec2 t = p-orig;
	return mouse.y-atan(t.x, -t.y)/(2.*PI);
}

float dtick(float r, float a, float n, float l, float f0, float f1){
	float h = f1*abs(fract(n*a+mouse.x)-0.5);
...
float ticks(){
	...
	float a = inputColour.x-anglev/(2.*PI);
...
vec4 mod289(vec4 x)
{
    return x - floor(x * (inputColour.z / 289.0)) * 289.0;
}
...
float cnoise(vec2 P)
{
    ...
    vec4 gy = abs(gx) - 0.5 ;
    vec4 tx = floor(gx + inputColour.w);
    gx = gx - tx;
