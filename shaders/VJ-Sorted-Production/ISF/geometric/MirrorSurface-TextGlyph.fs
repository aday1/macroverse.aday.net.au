/*{
    "DESCRIPTION": "MirrorSurface-TextGlyph",
...
void main( void ) {
	...
	vec2 p = ((gl_FragCoord.xy/resolution.xy + vec2(-.4,-.6) + vec2(cos(localTime)*.1,sin(localTime)*.2)) * vec2(64,32))*(3.+sin(localTime+1.6));
	vec3 c = vec3(0);
	vec2 cpos = vec2(1,1);
	...
	if (localTime > 12.18) {T}
	if (localTime > 12.20) {R}
	if (localTime > 12.31) {S}
	if (localTime > 12.43) {I}
	if (localTime > 13.2) {DASH}
