/*{
    "DESCRIPTION": "DotMatrix-OrganicForm-5",
...
vec4 live = vec4(3,3,3,1.);
vec4 dead = vec4(0.,0.,0.,1.);
vec4 blue = vec4(0.,0.,10.,1.);
...
void main( void ) {
	...
	if (length(position-mouse) < 0.01) {
		float rnd1 = mod(fract(sin(dot(position + time * 0.001, vec2(1.9898,7.233))) * 437.5453), 1.0);
		if (rnd1 > 0.4) {
			gl_FragColor = live;
		} else {
			gl_FragColor = blue;
