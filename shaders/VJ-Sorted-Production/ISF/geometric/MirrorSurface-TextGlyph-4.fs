/*{
    "DESCRIPTION": "MirrorSurface-TextGlyph-4",
...
void main( void ) {
	...
	vec2 p = ((gl_FragCoord.xy/resolution.xy + vec2(-.3,-.8) + vec2(cos(time)*.1,cos(time)*.0)) * vec2(64,32))*(3.+sin(time+1.6));
	vec3 c = vec3(0);
	vec2 cpos = vec2(21,1);
	...
	if (time > 12.6) {T}
	if (time > 12.8) {slashN}
	if (time > 12.18) {T R S I}
	if (time > 12.20) {slashN}
	if (time > 12.31) {P O O  B R A I N}
	if (time > 12.43) {slashN}
	if (time > 13.2) {L O O N I E S}
	if (time > 13.18) {slashN}
	if (time > 13.18) {S}
	if (time > 13.20) {C}
	if (time > 13.26) {E}
	if (time > 13.34) {N}
	...
	if (time > 15.) {slashN}
	if (time > 15.5) {J O I N COLON T R S A C}
	BLOCK slashN
