/*{
    "DESCRIPTION": "MirrorSurface-TextGlyph-3",
...
#endif
// ToDo :  logo.. from http://glslsandbox.com/e#26948.0
//Writer done..-Do Fix color- font fucked up?..... Harley / Mirror 
// Like dat? --novalis - 
//love it....need to fix the font and a new logo...
// Have fun @ trsac, too far away for me though --novalis (greets from germany)
...
void main( void ) {
	float localTime = mod(time, 3.14*6.);
	vec2 p = ((gl_FragCoord.xy/resolution.xy + vec2(-.3,-.8) + vec2(cos(localTime)*.1,cos(localTime)*.0)) * vec2(64,32))*(3.+sin(localTime+1.6));
	vec3 c = vec3(0);
	vec2 cpos = vec2(21,1);
	vec3 txColor = vec3(1.,1.6,1.4);
	slashN
	...
	if (localTime > 6.26) {O}
	if (localTime > 6.38) {R}
	if (localTime > 7.0) {DASH}
	if (localTime > 11.) {slashN}
	if (localTime > 11.8) {S}
	if (localTime > 12.) {A}
	if (localTime > 12.2) {L}
	if (localTime > 12.4) {U}
	if (localTime > 12.6) {T}
	if (localTime > 12.8) {slashN}
	if (localTime > 12.18) {T R S I}
	if (localTime > 12.20) {slashN}
	if (localTime > 12.31) {P O O  B R A I N}
	if (localTime > 12.43) {slashN}
	if (localTime > 13.2) {L O O N I E S}
	if (localTime > 13.18) {slashN}
	if (localTime > 13.18) {S}
	if (localTime > 13.20) {C}
	if (localTime > 13.26) {E}
	if (localTime > 13.34) {N}
	if (localTime > 13.46) {E}
	if (localTime > 13.53) {S}
	if (localTime > 13.63) {A}
	if (localTime > 13.73) {T}
	if (localTime > 13.82) {slashN}
	if (localTime > 14.) {slashN}
	if (localTime > 14.5) {G R E E Z COLON N O V A L I S}
	if (localTime > 15.) {slashN}
	if (localTime > 15.5) {J O I N COLON T R S A C}
	BLOCK slashN
	...
	vec2 tuv = uv / vec2(sin(localTime),1.) + vec2(0.,sin(localTime*3.)*.05-.1);
	vec2 cen = aspect/2.0;
	vec2 tcen = cen / vec2(sin(localTime),1.);

	vec3 color = vec3(0);

	vec3 tricol = retrograd(0.1, 0.8, uv + vec2(0.,sin(localTime*3.)*.05 - .1));
