/*{
    "DESCRIPTION": "StarField-TextGlyph-2",
...
void main( void ) 
{
	...
	ch_pos = ch_start + vec2(sin(time),1.0);  // set start position
       
	ch_color = vec3 (2.6 - 4.*mouse.x, 1.2-mouse.x*mouse.y, 0.5+mouse.y);
	R E A R R A N G E D    nl
	s_less _ S O U R C E C O D E _ s_greater nl
