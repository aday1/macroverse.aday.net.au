/*{
    "DESCRIPTION": "NyanSpaceMiddle2-Blue",
...
vec3 background(vec2 p)
{
	...
	float q = pm.x+pm.y;
	vec3 color = vec3(0,66,121)/255.0;
	...
	for (int i=0; i<15; i++)
	{
		...
		p2=p*s+vec2(iGlobalTime/s,s*16.0)-mod(p*s+vec2(iGlobalTime/s,s*1.0),vec2(0.05));
		stars=noise(p2*32.0);
...
vec4 rainbow(vec2 p)
{
	...
	p.x-=mod(p.x,0.00);
	float q = max(p.x,-0.0);
	float s = sin(p.x*(0.0)+iGlobalTime*0.0)*0.00;
	s-=mod(s,0.00);
	p.y+=s;
