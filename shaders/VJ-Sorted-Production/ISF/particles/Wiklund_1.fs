...
// http://glslsandbox.com/e#28407.0
// DEMOPARTAH
// SCROLLERBARS
...
vec2 position;
vec3 bars;
float barsize = mouse.x;
float barsangle = mouse.y;
...
void _userMain( void ) {
        ...
        //The bars
        {
            ...
            position = position * vec2(2.55) - vec2(0.3);    
        
            bars = vec3(0., 0., 0.);
            float t = mod(time * 0.1, 15.) + time;
        ...
        vec3 txColor = vec3(1);
        slashN
        DASH DASH DASH W I K L U N D DASH DASH DASH _ 
        slashN
        DASH DASH DASH W I K L U N D DASH DASH DASH  
        slashN
