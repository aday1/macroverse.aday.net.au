...
void bar(float pos, float r, float g, float b)
{
	 if ((position.y <= pos + barsize) && (position.y >= pos - barsize))
		color = mixcol(0.1 - abs(pos - position.y) / barsize, r, g, b);
}
...
void main()
{
    ...
    goTYPE ;
    {inWHT}_ _ A N D DASH T H E DASH B A N D EXCLAM EXCLAM EXCLAM _ _
	slashN ; 
...
initText();
	vec2 uv=q;
	vec2 tuv = uv + vec2(0.03 + sin((mouse.x-uv.x) * uv.y * 10.0 + time) * 0.03, 0.03 + cos(uv.y * uv.x * 8.0 + time) * 0.03);
