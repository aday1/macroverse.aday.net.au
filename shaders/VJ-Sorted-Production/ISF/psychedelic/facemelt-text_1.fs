...
#define _nl print_pos = vec2(start_pos.x,print_pos.y) - vec2(0,CHAR_SPACING.y);
...
int getDigit(float n, int i)
{
	float n2 = n/pow(10.0,float(i));
	return int(mod(n2, 10.0));
}
vec3 number(float n, vec2 uv)
{
    	vec3 col = vec3(0);
	if (n<0.){
		_dsh
	...
	if(n>3000000.) _hsh // not precise
	for(int i=8; i>=0; i--)
	{
		if (n >= pow(10.0,float(i)))
			col += digit(getDigit(n,i),uv);
	}
	_per
	for(int i=-1; i>=-4; i--)
	{
...
vec3 timer(vec2 uv)
{
	return number(time, uv);
}
vec3 char_map(vec2 uv)
{
    	vec3 col = vec3(0);
	_exc _quo _hsh _dol _pct _amp _apo _lbr _rbr _ast _crs _per _dsh _com _lsl _nl
	_0 _1 _2 _3 _4 _5 _6 _7 _8 _9 _nl
	_col _scl _les _equ _grt _que _ats _nl
	_F _A _C _E _spc _M _E _L _T _spc 
    	return col;
}
void _userMain( void )
{
