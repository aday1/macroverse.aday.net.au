/*{
    "DESCRIPTION": "PsychedelicSwimmingPool",
...
void main() {
    ...
    vec2 mpos = mouse;// mouse/resolution;
    mpos = vec2(0.5 - mpos.x, 0.5 - mpos.y) * sensibility;

    vec3 ro = vec3(sin(time*2.)*2.,cos(time*2.)*2.,time*10.);
    vec3 rd = normalize(vec3((-1.+2.*p)*ratio,2.5));

    rd.yz = rotate(rd.yz, mpos.y);
    rd.xz = rotate(rd.xz, mpos.x);
