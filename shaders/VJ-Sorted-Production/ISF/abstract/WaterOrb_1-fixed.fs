...
vec4 getdensity(vec3 pos)
{
    ...
    float localTime = time * TIME_SCALE;
    ...
    vec4 inner_color = vec4(colFromHue(cnoise(vec4(samplePos / 5.0, localTime / 15.0))) * 25.0, 1.0);
    ...
    float sample_ = (cnoise(vec4(samplePos * NOISINESS, localTime)) + 1.0) / 2.0;
