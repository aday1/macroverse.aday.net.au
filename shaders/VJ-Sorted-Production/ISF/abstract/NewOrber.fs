...
float scene(vec3 p)
{
	...
	p = rotatex(p,time*inputColour.y*0.1);
	p = rotatez(p,time*inputColour.w*0.1);
	float d1 =  length(p) - inputColour.x*1.0 + sin(-4.0*time+60.0*p.x)*0.01+sin(inputColour.y*time+50.0*p.y)*inputColour.z*-0.1;
	return d1;
}
