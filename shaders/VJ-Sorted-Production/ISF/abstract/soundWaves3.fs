...
vec3 shader( vec2 p, vec2 resolution ) {
	...
	float wave = 1.-abs(pos.y - (mouse.y*sin(mouse.x*3.*(cos(pos.x + cos(time) +(3.*sin(time /2.))))*PI*4.+cos(time))*0.25+1.)); // 'just string together a bunch of random stuff' remix
