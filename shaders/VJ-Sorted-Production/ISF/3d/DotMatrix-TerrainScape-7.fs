/*{
    "DESCRIPTION": "DotMatrix-TerrainScape-7",
...
float Map( vec3 p )
{
	...
	return max(length(p.xy) - 1.01, (-(length(p.xy)) * p.z/ length(p) )) / abs(scale);
}
...
void main(void)
{
	...
	if(t < 199.0)
	{
		...
		float sha = 1.;//clamp(Shadow(pos+nor*0.005, lightDir, lightDist) / max(lightDist-2.0, 0.1), 0.0, 1.0);
