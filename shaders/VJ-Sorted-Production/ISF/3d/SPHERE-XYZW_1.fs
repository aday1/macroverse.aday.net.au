...
void main()
{
	...
	{
		total_range	= mix(total_range, range, inputColour.y);   
		...
		color		+= hsv(pow(range, inputColour.z), range*inputColour.w, 1.) * response;
		color 		*= decay;
		...
		color 		+= pow(delta, 16.)*inputColour.x;
