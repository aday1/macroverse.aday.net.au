...
float supershape(vec2 p, float m, float n1, float n2, float n3, float a, float b, float s, float r) {
    ...
    ang += 5.0*sin(time*0.01+length(p)); // (.bpt.) silly term to spin and introduce some asymmetry
    float T = m * ang / 4.0;
...
#define SAMPLER(p) supershape(p - vec2(0, 0), 2.0+18.0*floor(sin(time)), 1.5*abs(sin(time*0.1)+0.5), 5.0*sin(time*0.5), 3.0*cos(time), 1.0, 1.0, 1.0, sin(time*1.2))
