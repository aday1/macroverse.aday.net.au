/*{
    "DESCRIPTION": "StupidBrokenLightBall",
...
vec3 background (vec3 r, float t) {
	vec3 light = normalize(vec3(cos(t), mouse.y, sin(t)));
	float sun = max(0.0, dot(r, light));
	float sky = max(0.0, dot(r, vec3(inputColour.x, mouse.x, inputColour.z)));
	float ground = max(inputColour.w, dot(r, vec3(inputColour.x, -mouse.y, inputColour.x)));
	return pow(sun, 250.0) * vec3(inputColour.x, inputColour.y, inputColour.w) + sky * vec3(inputColour.y, mouse.y, mouse.x) + ground * vec3(0.6, 0.4, 0.5);
}
