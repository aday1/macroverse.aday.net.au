...
float map(vec3 p){return 1.-(length(vec2(length(p.xz)-1.8,p.y))-.2);}
...
void _userMain(void){
	...
	float v=1.-trace(vec3(cos(time),sin(time*.1)*.5,sin(time)),vec3(uv,-.5))/FAR;
	gl_FragColor=vec4(mix(vec3(mouse.x,1.3,mouse.y),vec3(mouse.y,mouse.x,mouse.x),sin(time*.1)*.5+.5)*v,1);
}
