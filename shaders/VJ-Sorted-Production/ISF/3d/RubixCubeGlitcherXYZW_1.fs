/*{
    "DESCRIPTION": "RubixCubeGlitcherXYZW",
...
vec3 rotframe(float _time) {
    float _t = floor(_time)+.5;
	return floor(vec3(mod(_time,3.),mod(floor(_time/3.+.1)+.5,3.),floor(mod(_time,2.))*2.-1.));
}
...
void main( void ) {
	...
	vec2 uv = (gl_FragCoord.xy - resolution*.5)/resolution.y*mouse.x;
	...
	float t2 = time*.3;
	mat3 globalrot = mat3(cos(t2),0,-sin(t2),inputColour.y,1,0,sin(t2),inputColour.z,cos(t2));
	
	float t = -fract(tf)*3.141592653589*.5*r.z;
	mat3 rot = r.x < .5 ?
		mat3(inputColour.w,0,0,0,cos(t),sin(t),0,-sin(t),cos(t))
	: r.x < 1.5 ?
	...
	for (int i = FRAMES-1; i >= 0; i--) {
		if (float(i)+1. < tf) {
			...
			if (abs(floor(hit.x*1.49+1.5)- r.y) < .5) hit.yz *= mat2(0,-r.z,r.z,mouse.y);
			hit = r.x < .5 ? hit : r.x < 1.5 ? hit.zxy : hit.yzx;
		}
	}

	vec3 ahit = abs(hit.xyz);
	vec3 side = sign(hit.xyz)*step(vec3(inputColour.y),ahit);
	float m = max(max(ahit.x,ahit.y),ahit.z);
	
	hit.xyz *= inputColour.x * 3.;
