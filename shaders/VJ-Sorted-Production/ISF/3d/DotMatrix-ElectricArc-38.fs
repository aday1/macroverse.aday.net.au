/*{
    "DESCRIPTION": "DotMatrix-ElectricArc-38",
...
float supershape(vec2 p, float m, float n1, float n2, float n3, float a, float b, float s, float r) {
    ...
    float A = v * s;
    float B = length(p);
    return tan(A/B)-cos(A/B);
}

float HMM(float t,float b,float m) {
    return m*sin(1./t*t)*(sin(t*0.1)+b);
}
...
#define SAMPLER(p) 3.*cos(time*0.1)*SAMPLER_(p,-1.0) + (SAMPLER_(p*-1.5-0.5,1.0) + SAMPLER_(p*1.5-0.5,1.0))
#endif
