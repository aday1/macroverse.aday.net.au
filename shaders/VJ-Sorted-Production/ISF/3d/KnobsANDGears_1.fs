...
float hash(float n) 
{ 
    return fract(sin(n)*458.5453123); 
}

float _customNoise2(in vec2 x)
{
    vec2 p = floor(x);
    vec2 f = fract(x);
    f = f*f*(3.0-2.0*f);
...
float _customNoise3(in vec3 x)
{
    ...
    return mix(
        ...
        mix(mix(hash(n+1.0), hash(n+114.0),f.x), mix(hash(n+270.0), hash(n+271.0),f.x),f.y),f.z);
}
...
vec3 greeble3(vec3 p, float phase)
{
    ...
    float phase2 = _customNoise2(vec2(i));
    vec3 q = p;
    q.xz = mod(q.xz, 0.15);
    q.xz -= 0.075;
    q.y -= 0.5;
    ...
    float t = phase2 * 2.0 * pi + time * 8.0;
...
vec3 f( vec3 p )
{
    ...
    float hash = _customNoise2(vec2(h.xz));
    h = ivec3(p+42.0);
    float phase = _customNoise2(vec2(h.xz));
    vec3 q = p;
    q.xz = mod(q.xz, 1.0);
    q -= 0.5;
