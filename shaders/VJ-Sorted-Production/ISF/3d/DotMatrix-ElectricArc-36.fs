/*{
    "DESCRIPTION": "DotMatrix-ElectricArc-36",
...
#define AMBIENT_COLOR vec3(.2,.4,1.)
#define FLOOR_COLOR vec3(1.,.7,.9)
#define ENERGY_COLOR vec3(2.5,1.2,.8)
#define BRIGHTNESS .9
#define GAMMA 1.4
...
void main()
{
	...
	vec2 mpos =(mouse.xy/resolution.xy-.5)*3.;
	if (mpos.x<1.) mpos=vec2(0.);
...
#ifdef ENABLE_POSTPROCESS
	vec3 rain=vec3(0.);//pow(texture2D(inputImage,uv2+time*7.25468).rgb,vec3(1.5));
