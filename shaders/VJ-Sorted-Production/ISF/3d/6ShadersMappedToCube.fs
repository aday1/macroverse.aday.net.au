...
float iGlobalTime = time;  // 	lazy gigatron
...
vec3 Twister(vec3 p)
{
    float f = sin(iGlobalTime/3.)*1.45;
    float c = cos(f*p.y);
    float s = sin(f/2.*p.y);
    mat2  m = mat2(c,-s,s,c);
...
{
    p=Twister(p);
    cubevec.x = sin(iGlobalTime);
    cubevec.y = cos(iGlobalTime);
...
vec4 slice(float x0, float x1, vec2 uv)
{
    ...
    uv.y += iGlobalTime * XSCROLL_SPEED; //Scrolling
