...
vec3 render( vec3 ro, vec3 rd ) {
	...
	vec3 color = vec3(0.6,0.2,0.3); // base color is sky color
...
void main( void ) {
	...
	// camera
	float x = 0.0 + mouse.x; // (0.0005*time);
	float y = 0.0 ;
	float z = 0.0 + mouse.y ; // sin(0.1*time);
	vec3 cPos = vec3(x, y, z); // position
	cPos.y = terrain(cPos.xz, 1) + 1.5;
	
	const vec3 cUp = vec3(0., 1., 0.); // up 
	vec3 cLook = vec3(cPos.x + 1.0, cPos.y*1.0, 0.0); // lookAt
