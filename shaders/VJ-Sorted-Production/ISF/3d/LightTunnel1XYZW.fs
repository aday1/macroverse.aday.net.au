...
void main( void )
{
	...
	float angleDist = (angleSquareDist+inputColour.x);
	float adist = angleDist/rad*kStarDensity;
	float dist = abs(fract((t*.1+adist))-.5);
	float white1 = max(0.,inputColour.z - dist * 100.0 / (kMotionBlur*speed+adist));
	float white2 = max(0.,(inputColour.w-.5*cos(k2PI * angleFract))*1./max(0.6,2.*adist*angleDist));
	...
	float nebD1 = inputColour.x/rad + 4.5*(1.0 + sin(1.1 + 3.0*A + 0.71*cos(2.0*A)));
	float nebD2 = inputColour.w/rad + 3.7*(1.0 + sin(3.7 + 2.0*A + 0.46*sin(3.0*A)));
	float R1 = 1.0 * rad * (1.0 + sin(0.3+3.0*A + 2.4 * cos(0.2+3.0*A)*sin(2.1+0.42*(nebD1+speed*time)) + sin(2.0*6.283*position.x) ));
	float R2 = inputColour.x * rad * (1.0 + sin(1.1+4.0*A + 3.2 * cos(0.7+4.0*A)*sin(1.7+0.27*(nebD2+speed*time)) + cos(3.0*6.283*position.y) ));
	float P1 = 0.5 + inputColour.w*sin(5.7*position.x+.22*(speed*time));
	float P2 = 0.5 + inputColour.w*sin(4.44*position.y+.17*(speed*time)) ;
	color.r += inputColour.x*R1 + 0.3*R2 + 0.1*P1*P2;
	color.b += inputColour.y*R1 + 0.8*R2 + .1*P2*R1;
	color.g += inputColour.z*R1*R2*P2;
