/*{
    "DESCRIPTION": "RaymarchPrimitive-Fire-FrostCrystal",
...
void main(void)
{
    ...
    vec3 col = texture2D( inputImage, st ).xyz;
    float w = col.x;

    col *= 1.0 - texture2D( inputImage, 0.4*uv + 0.1*col.xy  ).xyy;
    col *= w*2.0;
...
vec2 hash2( vec2 p )
{
    ...
    // return texture2D( inputImage, (p+0.5)/256.0, -100.0 ).xy;
