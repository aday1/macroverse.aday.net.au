/*{
    "DESCRIPTION": "DotMatrix-StrobeLight",
...
vec3 effectIn(float _ftime) {
	float begin = 0.0;
	float end = 2.0/2.0;
	float offset = -1.5/2.0;
	if (_ftime < begin) return vec3(0.0);
	if (_ftime > end) return vec3(0.0);
	float delta = (_ftime+offset)*2.0;
...
vec3 effectOut(float _ftime) {	
	float begin = 2.0/2.0;
	float end = 2.0/2.0+3.0/2.0;
	float offset = -1.0/2.0;
	if (_ftime < begin) return vec3(0.0);
	if (_ftime > end) return vec3(0.0);
	float delta = (-_ftime-offset+begin)*2.0;
