...
# Targets launcher.ps1 (GUI); launcher refreshes shortcut on each run.
...
if (-not (Test-Path $launcherCliPs1)) {
    Write-Host "launcher-cli.ps1 not found." -ForegroundColor Red
    exit 1
}
...
# Main = full launcher (CLI menu). Server only = run exe in foreground in full-screen console. Both open maximized shell.
$shortcuts = @(
    @{
        Name = "Macroverse - Wired Atelier"
        Description = "Version 42. Full launcher (shell menu): Launch/Kill/New session, Open browser, Rebuild, Reindex, Regen thumbnails, Hard reset DB, Factory reset, Clear cache, Update, Update shortcuts, NUKE, Fix failed shaders."
...
Write-Host "  Macroverse - Wired Atelier = shell menu (full-screen).  Server only = exe in foreground (full-screen)." -ForegroundColor Gray
