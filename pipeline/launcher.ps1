...
$btnPull = AddButton "Update" (12 + 2*($btnW + $gap)) $y $cUpdate {
    Get-Process -Name "Macroverse42" -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
    Start-Sleep -Milliseconds 300
    $statusLabel.Text = "Killed sessions. Running git pull + rebuild + update shortcuts..."
    Start-Process powershell -ArgumentList "-NoProfile -ExecutionPolicy Bypass -NoExit -Command `"Set-Location '$repoRoot'; git pull; & '$buildPs1'; & '$createShortcut'`"" -WorkingDirectory $repoRoot
}
