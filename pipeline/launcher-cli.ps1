...
function Do-Update {
    Get-Process -Name "Macroverse42" -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
    Start-Sleep -Milliseconds 300
    Write-Host "Running git pull + rebuild + update shortcuts..." -ForegroundColor Cyan
    Set-Location $repoRoot
    git pull 2>&1 | Out-Null
    if ($LASTEXITCODE -ne 0) {
        Write-Host "git pull failed (exit code $LASTEXITCODE)." -ForegroundColor Red
        exit $LASTEXITCODE
    }
    & $buildPs1
    & $createShortcut
...
while ($true) {
    ...
    Write-Host "  A. Factory reset             B. Clear cache      C. Update (pull+rebuild+shortcuts)"
