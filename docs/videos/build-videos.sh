...
#
# Requires: ffmpeg 6+ with lavfi (mandelbrot, life, cellauto, gradients,
# color, drawtext, geq) and libx264. The ImageMagick/cwebp toolchain is
...
ffmpeg -y -hide_banner -loglevel error \
  ...
  -filter_complex "
    ...
    [a][b]xfade=transition=fade:duration=2:offset=3[xf];
...
ffmpeg -y -hide_banner -loglevel error \
  ...
  -f lavfi -i "gradients=size=${W}x${H}:rate=${FPS}:duration=8:c0=0x0d0820:c1=0x06040f:c2=0x150a30:nb_colors=3:speed=0.0008" \
  -filter_complex "
...
ffmpeg -y -hide_banner -loglevel error \
  ...
  -f lavfi -i "gradients=size=${TW}x${TH}:rate=${FPS}:c0=0x06040f:c1=0x8833cc:c2=0x00ddaa:c3=0xee8833:nb_colors=4:speed=0.005" \
  -filter_complex "
    ...
    [4:v]scale=${DW}:${DH}:flags=bicubic,format=rgba,trim=duration=8,setpts=PTS-STARTPTS,colorize=hue=180:saturation=0.7:lightness=0.5[t4];
    [5:v]scale=${DW}:${DH}:flags=bicubic,format=rgba,trim=duration=8,setpts=PTS-STARTPTS,colorize=hue=30:saturation=0.7:lightness=0.5[t5];
    [6:v]scale=${DW}:${DH}:flags=bicubic,format=rgba,trim=duration=8,setpts=PTS-STARTPTS,colorize=hue=160:saturation=0.7:lightness=0.5[t6];
...
ffmpeg -y -hide_banner -loglevel error \
  ...
  -filter_complex "
    [0:v]format=rgba,
      drawbox=x=40:y=40:w=${W}-80:h=180:color=0x0d0820@0.95:t=fill,
      ...
      drawbox=x=40:y=320:w=${W}-80:h=180:color=0x0d0820@0.95:t=fill:enable='gte(t,3.0)',
...
ffmpeg -y -hide_banner -loglevel error \
  ...
  -filter_complex "
    [0:v]format=rgba,
      ...
      drawbox=x=80:y=220:w=240:h=140:color=0x0d0820:t=fill,
      ...
      drawbox=x=80:y=220:w=240:h=140:color=0x00ddaa@0.30:t=fill:enable='gte(t,1.0)',
      drawtext=fontfile='${FONT_HEAD}':text='OK':x=80+240-58:y=222:fontsize=22:fontcolor=0x00ddaa:enable='gte(t,1.2)',

      drawbox=x=360:y=220:w=240:h=140:color=0x0d0820:t=fill,
      ...
      drawbox=x=360:y=220:w=240:h=140:color=0x4488cc@0.10:t=fill:enable='between(t,2.5,4.5)',

      drawbox=x=640:y=220:w=240:h=140:color=0x0d0820:t=fill,
      ...
      drawbox=x=640:y=220:w=240:h=140:color=0x4488cc@0.10:t=fill:enable='gte(t,5.0)',
