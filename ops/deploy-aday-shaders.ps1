# Sync Macroversed-FortyTwoEdition shaders to macroverse-aday (password lane) on Linode.
...
if ($triggerReindex) {
  Write-Host "Triggering shader reindex on aday lane (may take several minutes)..."
  try {
    $cred = "aday:macroverse"
    $pair = [Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes($cred))
    $headers = @{ Authorization = "Basic $pair" }
    Invoke-RestMethod -Uri "https://macroverse-aday.aday.net.au/api/native-scan" -Method Post -Headers $headers -TimeoutSec 600 | Out-Null
    Write-Host "Reindex request accepted."
  } catch {
    Write-Host "Reindex call failed (library is still on disk): $($_.Exception.Message)"
    Write-Host "Open https://macroverse-aday.aday.net.au and use Settings -> scan, or POST /api/native-scan"
  }
}

Remove-Item -Recurse -Force $staging
Write-Host "Done. Open https://macroverse-aday.aday.net.au (aday / macroverse)"
