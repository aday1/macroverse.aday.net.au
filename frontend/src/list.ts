...
import { hasDesktopShell } from './hostCapabilities.js';
...
import { stopThumbnailLoad, isThumbnailsLoading, resumeThumbnailLoad, hasMissingThumbnails, getThumbnailProgress, enqueueAllMissingThumbnails } from './init/loadSequence.js';
import type { IndexEntry } from './types.js';
import { attachShaderListDrag, dispatchVjDeckLoad } from './shaderPointerDrag.js';
...
export function buildList(): void {
  ...
  filtered.forEach((entry: IndexEntry, i: number) => {
    ...
    if (entry.path) {
      attachShaderListDrag(item, entry.path, name);
    }
    ...
    item.onclick = () => {
      if (item.dataset.shaderPointerDrag === '1') return;
      selectShader(entry, item);
    };
...
function showShaderContextMenu(ev: MouseEvent, entry: IndexEntry, itemEl: HTMLElement): void {
  ...
  const sepVj = document.createElement('div');
  sepVj.style.cssText = 'height:1px;background:var(--bevel-dark);margin:4px 0;';
  menu.appendChild(sepVj);
  addItem('Load to VJ Deck A', () => {
    if (entry.path) dispatchVjDeckLoad('A', entry.path);
  });
  addItem('Load to VJ Deck B', () => {
    if (entry.path) dispatchVjDeckLoad('B', entry.path);
  });
...
export function initListFilters(): void {
  ...
  if (toolbar) {
    ...
    if (!generateThumbsBtn) {
      ...
      generateThumbsBtn.title = 'Generate missing thumbnails when the browser is idle';
      generateThumbsBtn.style.cssText = 'display:none;margin-bottom:6px;';
      generateThumbsBtn.onclick = () => {
        const allWithout = entries.filter((e) => e.path && !getThumbnail(e.path!));
        if (allWithout.length === 0) {
          status('All shaders already have thumbnails.');
          return;
        }
        status('Queued ' + allWithout.length + ' thumbnails for idle generation…');
        enqueueAllMissingThumbnails(false);
      };
