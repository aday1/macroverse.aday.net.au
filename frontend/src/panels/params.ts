...
import { audioEngine, autoAssignFftBandMap, FFT_BAND_LABELS, FFT_BAND_COUNT, isFftAutoSelectEnabled, setFftAutoSelectEnabled } from '../engines/audio.js';
...
export function syncRoliblockFromView(): void {
  ...
  if (mgr.ledDisplayMode === 'sharedOutput') {
    for (const dev of mgr.getDevices()) {
      ...
      dev._onMouse = (x, y) => {
        if (activeView === 'vj') setVjMouseFromRoliblock(x, y);
        else if (activeView === 'gallery') setGalleryRoliblockMouse(x, y);
        else setMouse(x, y);
      };
...
export function buildParamsPanel(entry: IndexEntry | null): void {
  ...
  if (isFftAutoSelectEnabled()) {
    maybeAutoArrangeMainFft(params.filter((p) => p.type === 'float' || p.type === 'bool'));
    rebuildExternalMappingUI(params);
  }
...
function initExternalSections(): void {
  ...
  wrap.innerHTML = `
    ...
    <div class="external-section">
      ...
      <div class="external-body open" data-section="audio">
        ...
        <label style="display:flex;align-items:center;gap:6px;font-size:10px;color:var(--crt-dim);margin-bottom:4px;cursor:pointer;user-select:none">
          <input type="checkbox" id="fftAutoSelectCheck" checked autocomplete="off" title="When on, new shaders auto-map FFT bands to params (sources and effects)">
          Auto-select bands for new shaders
        </label>
      ...
      <div class="external-body" data-section="roliblock">
        ...
        <div class="external-row" style="margin-top:6px;align-items:center;gap:6px">
          ...
          <select id="roliblockLedDisplayMode" style="font-size:10px;background:var(--amiga-bg);border:1px solid var(--bevel-dark);color:var(--crt-fg);flex:1">
            <option value="sharedOutput">Shared output / Mirror</option>
  ...
  function roliblockModeLabel(mode: LedDisplayMode): string {
    if (mode === 'sharedOutput') return 'Shared output';
    if (mode === 'independent') return 'Independent decks';
    if (mode === 'stretched') return 'Combined wide';
    return 'Linked XY';
  }

  function roliblockDeviceStatusLabel(dev: RoliblockDevice): string {
    if (dev.connectionType === 'ble' && !dev.bleConnection) return 'Re-pair required';
    if (!dev.enabled) return 'Disabled';
    if (dev.isReady()) return 'LED ready';
    if (dev.hasTouchInput() && !dev.hasLedOutput()) return 'Touch only / no LED output';
    if (dev.hasLedOutput() && !dev.handshakeDone) return 'Handshaking...';
    if (dev.hasTouchInput()) return 'Touch ready';
    return 'Configured';
  }
  ...
  function renderDeviceSlotHtml(dev: RoliblockDevice, mgr: typeof roliblockManager): string {
    ...
    html += '<span class="roli-dev-status" data-dev-id="' + dev.id + '" style="font-size:9px;color:var(--crt-dim)">' + roliblockDeviceStatusLabel(dev) + '</span>';
    html += '</div>';
    // LED Processing
  ...
  function updateRoliblockStatus(): void {
    ...
    const touchOnly = devices.filter((d) => d.enabled && d.hasTouchInput() && !d.hasLedOutput()).length;
    const rePair = devices.filter((d) => d.connectionType === 'ble' && !d.bleConnection).length;
    const roliInputs = mgr.inputs.filter((i) => mgr.isRoliblockLike(i.name)).length;
    const roliOutputs = mgr.outputs.filter((o) => mgr.isRoliblockLike(o.name)).length;
    const usbPairs = Math.min(roliInputs, roliOutputs);
    const mode = roliblockModeLabel(mgr.ledDisplayMode);
    if (roliblockLedDisplayMode && roliblockLedDisplayMode.value !== mgr.ledDisplayMode) {
      roliblockLedDisplayMode.value = mgr.ledDisplayMode;
    }
    if (total === 0) {
      if (usbPairs > 0) {
        roliblockStatus.textContent = usbPairs + ' USB pair(s) found. Auto-detect to auto-rig shared output.';
      } else if (roliInputs > 0 || roliOutputs > 0) {
        roliblockStatus.textContent = 'Partial Roli USB seen: ' + roliInputs + ' touch / ' + roliOutputs + ' LED. Browser may expose only one side.';
      } else {
        roliblockStatus.textContent = 'Plug in USB then Request MIDI, or Pair BLE. Chrome/Edge only.';
      }
    } else if (count === 0) {
      roliblockStatus.textContent = total + ' device(s) configured. Enable to activate. Mode: ' + mode + '.';
    } else {
      const extras: string[] = [];
      if (touchOnly > 0) extras.push(touchOnly + ' touch only / no LED output');
      if (rePair > 0) extras.push(rePair + ' BLE re-pair required');
      roliblockStatus.textContent = ready + '/' + total + ' LED ready, ' + count + ' enabled. Mode: ' + mode + (extras.length ? '. ' + extras.join(', ') + '.' : '.');
    }
  }
  ...
  async function doRoliblockRequest(): Promise<void> {
    ...
    if (mgr.inputs.length === 0 && mgr.outputs.length === 0) {
      ...
      const usbPairs = Math.min(roliInputs, roliOutputs);
      if (usbPairs === 1) {
        status('Roliblock: 1 USB pair found. Shared-output auto-rig active; browser exposes one block.');
      } else if (usbPairs > 1) {
        status('Roliblock: ' + usbPairs + ' USB pairs found. Shared-output auto-rig active.');
      } else if (roliInputs > 0 || roliOutputs > 0) {
        status('Roliblock: partial USB Roli seen (' + roliInputs + ' touch / ' + roliOutputs + ' LED). Touch-only or LED-only until both sides appear.');
      } else {
  ...
  if (roliblockPairBleBtn) {
    roliblockPairBleBtn.addEventListener('click', async () => {
      ...
      if (ok) {
        roliblockManager.ledDisplayMode = 'sharedOutput';
        dev.deckAssignment = 'shared';
        dev.enable();
        roliblockManager.saveAll();
        if (roliblockLedDisplayMode) roliblockLedDisplayMode.value = roliblockManager.ledDisplayMode;
  ...
  if (roliblockManager.getDevices().length > 0) {
    renderRoliblockDeviceList();
    syncRoliblockFromView();
  }
  updateRoliblockStatus();
  ...
  if (audioStartBtn) {
    audioStartBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        maybeAutoArrangeMainFft(currentParamsMeta.filter((p) => p.type === 'float' || p.type === 'bool'));
        rebuildExternalMappingUI(currentParamsMeta);
      } catch (e) {
  ...
  const fftAutoSelectCheck = el('fftAutoSelectCheck') as HTMLInputElement | null;
  if (fftAutoSelectCheck) {
...
function maybeAutoArrangeMainFft(assignable: ParamDef[]): void {
  if (!isFftAutoSelectEnabled()) return;
  const ids = assignable.map((p) => p.id);
  autoAssignFftBandMap(audioEngine.bandParamMap, ids);
