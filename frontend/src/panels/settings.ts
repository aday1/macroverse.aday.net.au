...
import { buildGigAudienceStreamUrl, buildGigJoinUrl, buildGigOutputUrl, buildGigVrAudienceUrl, buildGigVrControllerUrl, drawGigQr, refreshGigQrPair } from '../gigQr.js';
...
import { isCompactLayout } from '../init/layoutTier.js';
import { clearLocalStore, exportLocalStore, getLocalStoreStats, importLocalStore } from '../cloudLocalStore.js';
...
function getPanelContainer(): HTMLElement {
  ...
  panelEl.className = 'settings-panel mv-sheet-panel';
  panelEl.style.cssText = 'display:none;';
...
function getOverlay(): HTMLElement {
  ...
  overlayEl.className = 'settings-overlay mv-sheet-scrim';
  overlayEl.style.cssText = 'display:none;';
...
function closePanel(): void {
  ...
  document.body.classList.remove('mv-settings-open');
}

function openPanel(): void {
  const p = getPanelContainer();
  const o = getOverlay();
  p.classList.toggle('mv-sheet-panel--phone', isCompactLayout());
  p.style.display = 'flex';
  o.style.display = 'block';
  document.body.classList.add('mv-settings-open');
  renderPanel();
}
...
async function renderPanel(): Promise<void> {
  ...
  body.innerHTML = `
    ...
    <div class="settings-section" style="margin-bottom:20px; display:none;" id="settingsCloudLocalSection">
      <div style="color:var(--amiga-copper); font-size:11px; text-transform:uppercase; margin-bottom:8px;">Browser-local edits (cloud)</div>
      <div id="settingsCloudLocalStats" style="font-size:10px; color:var(--crt-dim); margin-bottom:10px;">Server library is read-only. Shader code, tags, favorites, and new shaders you create are stored in this browser only.</div>
      <div style="display:flex; flex-wrap:wrap; gap:8px;">
        <button type="button" id="settingsCloudLocalExport" style="font-size:10px; padding:6px 10px; background:var(--amiga-surface); color:var(--amiga-copper); border:1px solid var(--bevel-dark); cursor:pointer;">Export JSON backup</button>
        <button type="button" id="settingsCloudLocalImport" style="font-size:10px; padding:6px 10px; background:var(--amiga-surface); color:var(--crt-fg); border:1px solid var(--bevel-dark); cursor:pointer;">Import backup</button>
        <button type="button" id="settingsCloudLocalClear" style="font-size:10px; padding:6px 10px; background:var(--amiga-surface); color:#ff8866; border:1px solid var(--bevel-dark); cursor:pointer;">Clear browser edits</button>
      </div>
      <input type="file" id="settingsCloudLocalImportFile" accept="application/json,.json" style="display:none" />
    </div>
    <div class="settings-section desktop-only" style="margin-bottom:20px;">
    ...
    <div class="settings-section desktop-only" style="margin-bottom:20px;" id="settingsDebugSection">
    ...
    <div class="settings-section desktop-only" style="margin-bottom:20px;" id="settingsGithubSection">
    ...
    <div class="settings-section" style="margin-bottom:20px;" id="settingsVjSessionSection">
      ...
      <div style="display:flex; flex-wrap:wrap; gap:16px; margin-top:10px; align-items:flex-start;">
        ...
        <div style="text-align:center;">
          <div style="font-size:9px; color:var(--crt-dim); margin-bottom:4px;">VR audience (WebXR dome)</div>
          <canvas id="settingsVjSessionQrVrAudience" width="180" height="180" style="background:#fff; padding:4px; border:1px solid var(--bevel-dark);"></canvas>
          <button type="button" id="settingsVjSessionQrCopyVrAudience" style="font-size:9px; margin-top:4px; padding:4px 8px; cursor:pointer;">Copy VR audience link</button>
        </div>
        <div style="text-align:center;">
          <div style="font-size:9px; color:var(--crt-dim); margin-bottom:4px;">VR VJ controller</div>
          <canvas id="settingsVjSessionQrVrVj" width="180" height="180" style="background:#fff; padding:4px; border:1px solid var(--bevel-dark);"></canvas>
          <button type="button" id="settingsVjSessionQrCopyVrVj" style="font-size:9px; margin-top:4px; padding:4px 8px; cursor:pointer;">Copy VR VJ link</button>
        </div>
      </div>
    </div>
    <div class="settings-section" style="margin-bottom:20px;" id="settingsVjControllerSection">
      ...
      <div style="display:flex; align-items:center; gap:8px; margin-bottom:8px;">
        ...
        <select id="settingsVjMidiTemplate" style="padding:6px 8px; font-size:10px; background:var(--amiga-bg); color:var(--crt-fg); border:1px solid var(--bevel-dark); font-family:inherit;">
          ...
          <option value="apc40_mk2">APC40 / Akai performance</option>
          <option value="custom">Custom (keep current)</option>
        </select>
      </div>
      <div style="font-size:9px; color:var(--crt-dim); margin-bottom:6px;">AKAI/APC: grid and scene buttons load Deck A; hold Shift for Deck B. Track controls map Deck A params, Device controls map Deck B params, faders follow Shift, crossfader is live, arrows/page CCs bank A/B.</div>
    ...
    <div class="settings-section desktop-only" style="margin-top:24px; padding-top:16px; border-top:1px solid var(--bevel-dark);">
  ...
  const qrVrAudienceCanvas = panel.querySelector('#settingsVjSessionQrVrAudience') as HTMLCanvasElement | null;
  const qrVrVjCanvas = panel.querySelector('#settingsVjSessionQrVrVj') as HTMLCanvasElement | null;
  ...
  if (vjSessionInput) {
    ...
    const refreshHint = () => {
      void ensureVjTokens(sessionForQr())
        .then(async () => {
          ...
          await refreshGigQrPair(qrJoinCanvas, qrOutputCanvas, qrJoinUrlEl, sessionForQr());
          const sid = sessionForQr();
          if (qrVrAudienceCanvas) await drawGigQr(qrVrAudienceCanvas, buildGigVrAudienceUrl(sid), 180);
          if (qrVrVjCanvas) await drawGigQr(qrVrVjCanvas, buildGigVrControllerUrl(sid), 180);
        })
        .catch(() => {
          if (vjSessionHint) {
    ...
    panel.querySelector('#settingsVjSessionQrCopyVrAudience')?.addEventListener('click', () => {
      void navigator.clipboard?.writeText(buildGigVrAudienceUrl(sessionForQr()));
    });
    panel.querySelector('#settingsVjSessionQrCopyVrVj')?.addEventListener('click', () => {
      void navigator.clipboard?.writeText(buildGigVrControllerUrl(sessionForQr()));
    });
  }
  ...
  function renderVjMidiMapList(container: HTMLElement): void {
    ...
    clipRow.innerHTML = '<span style="min-width:140px;">Clip / scene launch</span><span style="color:var(--crt-dim);">Grid=Deck A, Shift+grid=Deck B</span>';
  ...
  const cloudLocalSection = panel.querySelector('#settingsCloudLocalSection') as HTMLElement | null;
  const cloudLocalStats = panel.querySelector('#settingsCloudLocalStats') as HTMLElement | null;
  const cloudLocalExport = panel.querySelector('#settingsCloudLocalExport') as HTMLButtonElement | null;
  const cloudLocalImport = panel.querySelector('#settingsCloudLocalImport') as HTMLButtonElement | null;
  const cloudLocalClear = panel.querySelector('#settingsCloudLocalClear') as HTMLButtonElement | null;
  const cloudLocalImportFile = panel.querySelector('#settingsCloudLocalImportFile') as HTMLInputElement | null;
  if (cloudLocalSection && usesLocalBrowserStore()) {
    cloudLocalSection.style.display = 'block';
    getLocalStoreStats().then((s) => {
      if (!cloudLocalStats) return;
      cloudLocalStats.textContent =
        'Server library is read-only. Stored in this browser: ' +
        s.shaderEdits + ' code edit(s), ' + s.created + ' new shader(s), ' +
        s.deleted + ' hidden, ' + s.thumbnails + ' thumbnail(s). Export before clearing site data.';
    }).catch(() => {});
    cloudLocalExport?.addEventListener('click', () => {
      exportLocalStore().then((json) => {
        const blob = new Blob([json], { type: 'application/json' });
        const a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
        a.download = 'macroverse-browser-edits-' + new Date().toISOString().slice(0, 10) + '.json';
        a.click();
        URL.revokeObjectURL(a.href);
        status('Browser edits exported');
      }).catch((e) => status('Export failed: ' + (e as Error).message, true));
    });
    cloudLocalImport?.addEventListener('click', () => cloudLocalImportFile?.click());
    cloudLocalImportFile?.addEventListener('change', () => {
      const file = cloudLocalImportFile.files?.[0];
      if (!file) return;
      const reader = new FileReader();
      reader.onload = () => {
        const merge = confirm('Merge with existing browser edits? Cancel = replace all local edits.');
        importLocalStore(String(reader.result || ''), merge)
          .then(() => {
            status('Browser edits imported');
            loadSequence().catch(() => {});
            renderPanel();
          })
          .catch((e) => status('Import failed: ' + (e as Error).message, true));
      };
      reader.readAsText(file);
      cloudLocalImportFile.value = '';
    });
    cloudLocalClear?.addEventListener('click', () => {
      if (!confirm('Clear all browser-local shader edits? Server library is unchanged.')) return;
      clearLocalStore().then(() => {
        status('Browser edits cleared');
        loadSequence().catch(() => {});
        renderPanel();
      }).catch((e) => status('Clear failed: ' + (e as Error).message, true));
    });
  }
