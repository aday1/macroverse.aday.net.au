...
async function loadData(): Promise<void> {
  ...
  try {
    ...
    status(entries.length + ' shaders, ' + libraryEntries.length + ' wire patches' +
      (libraryEntries.length === 0 ? ' — select shaders and Generate .wire, or use Gen Sources' : ''));
  } catch (err) {
        ...
        if (successArea) {
          successArea.innerHTML = `<div class="wire-success-card">
            ...
            <div class="wire-success-actions">
              ...
              <button type="button" class="wire-btn desktop-only" id="wireSuccessExplore" title="Show in Explorer">Show in Explorer</button>
        ...
        if (successArea) {
          ...
          successArea.innerHTML = `<div class="wire-success-card">
            ...
            <div class="wire-success-actions">
              ...
              ${avcPath ? `<button type="button" class="wire-btn desktop-only" id="wireAvcExplore" title="Show in Explorer">Show in Explorer</button>` : ''}
            </div>
          </div>`;
          if (avcPath) {
  ...
  list.innerHTML = visible.map((e) => {
    ...
    return `<div class="wire-library-row ${isTemplate ? 'wire-lib-template' : ''}" data-wl-name="${esc(e.name)}" data-wl-path="${esc(e.path)}">
      ...
      <div class="wire-lib-actions">
        ...
        <button type="button" class="wire-btn wire-lib-explore desktop-only" title="Show in Explorer" data-explore-path="${esc(e.path)}">Explore</button>
  ...
  if (list) {
    ...
    list.addEventListener('contextmenu', (ev) => {
      ...
      const items = [
        ...
        ...(hasDesktopShell() ? [{ label: 'Show in Explorer', action: () => postOpenInExplorer({ path: filePath }).then(() => status('Opened in Explorer')).catch((e: Error) => status('Explorer: ' + e.message, true)) }] : []),
