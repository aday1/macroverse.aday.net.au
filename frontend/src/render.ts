...
let activeRenderLoop: ((t: number) => void) | null = null;
const quadVerts = new Float32Array([-1, -1, 1, -1, -1, 1, 1, 1]);

if (typeof document !== 'undefined') {
  document.addEventListener('visibilitychange', () => {
    const loop = activeRenderLoop;
    if (!loop) return;
    if (document.hidden) {
...
function sendMacroCamFrame(): void {
  if (!hasVideoOutput() || !macroCamEnabled || !gl || !gl.canvas) return;
  macroCamFrameCount++;
...
export function setPreviewCompileErrorOverlayVisible(visible: boolean): void {
  const overlay = el('previewCompileErrorOverlay');
  const wrap = el('canvasWrap') ?? el('canvas')?.parentElement;
  if (overlay) (overlay as HTMLElement).style.display = visible ? 'flex' : 'none';
  if (wrap) wrap.classList.toggle('preview-has-compile-error', visible);
}
...
export function render(fragSrc: string): void {
  ...
  if (emptyOrNoMain) {
    ...
    setPreviewCompileErrorOverlayVisible(true);
