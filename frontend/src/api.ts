...
export async function postThumbnailSave(payload: { path: string; dataUrl: string }): Promise<{ ok: boolean }> {
  const res = await fetch('/api/thumbnail', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ path: (payload.path || '').replace(/\\/g, '|'), dataUrl: payload.dataUrl || '' })
  });
...
export async function postShaderSave(payload: { path: string; content: string }): Promise<{ ok: boolean }> {
