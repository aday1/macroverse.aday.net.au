...
import { registerLocalShaderEntry } from './cloudLocalStore.js';
  ...
  createBtn.onclick = async () => {
    ...
    try {
      ...
      if (template === 'glsl') content = BLANK_GLSL;
        ...
        if (message && styleName === 'core-text-custom.fs') content = injectTextIntoCustomSource(content, message);
      ...
      if (usesLocalBrowserStore()) {
        await registerLocalShaderEntry(fullPath.replace(/\//g, '|'));
      }
      status(usesLocalBrowserStore() ? 'Created in this browser.' : 'Created. Reindexing...');
      overlay.remove();
      if (!usesLocalBrowserStore()) await postNativeScan();
      await loadSequence();
