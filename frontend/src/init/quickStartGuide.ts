...
import { isCompactLayout, isLayoutPhone } from './layoutTier.js';
...
export function showQuickStartGuide(force = false): void {
  ...
  overlay.className = 'quick-start-overlay' + (isCompactLayout() ? ' quick-start-overlay--compact' : '');
  ...
  subtitle.textContent = isLayoutPhone()
    ? 'Phone / tablet layout — bottom tabs drive Library, Preview, Code, Params, and More.'
    : isCompactLayout()
      ? 'Touch layout — use the bottom bar or Lib / Params in the top bar.'
      : 'New here? This machine has not opened Macroverse before.';
...
<ol class="quick-start-steps">
  <li><b>Library</b> — bottom tab or Lib button opens the shader list.</li>
  <li><b>Preview / VJ</b> — Preview tab for one shader; More → VJ for live A/B mixing and gig QRs.</li>
  <li><b>Params</b> — bottom Params tab or sliders dock for the active shader.</li>
  <li><b>Save</b> — top bar Save (cloud saves in this browser only).</li>
  <li><b>More</b> — commands, settings, help; M42 logo also opens Settings.</li>
</ol>
<p class="quick-start-note">Add to Home Screen in Safari for a full-screen iOS web app. Wire Hub and Pipeline need the desktop app.</p>`;
  } else {
    body.innerHTML = `
<ol class="quick-start-steps">
