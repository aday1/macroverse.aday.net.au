...
const VIEW_STORAGE_KEY = 'macroverse:view';
...
export function initCodeEditor(): void {
  ...
  viewTabs.forEach((tab) => {
    tab.addEventListener('click', () => {
      ...
      document.body.classList.toggle('view-vj', view === 'vj');
      ...
      if (view === 'preview') {
        ...
        if (!hasWirePipeline()) {
          status('Pipeline needs a local Macroverse install (Wire / Resolume)', true);
          return;
        }
        contentSplit.style.display = 'none';
        ...
        if (!hasWirePipeline()) {
          status('Wire Hub needs a local Macroverse install (Resolume Wire)', true);
          return;
        }
        contentSplit.style.display = 'none';
  ...
  saveBtn?.addEventListener('click', () => {
    ...
    postShaderSave({ path: path.replace(/\\/g, '|'), content })
      .then((res) => {
        ...
        status(res.local ? 'Saved in this browser (server copy unchanged).' : 'Saved. Git commit when EnableGit.');
