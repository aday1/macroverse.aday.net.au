...
export async function run(): Promise<void> {
  ...
  fetchVersion()
    .then((v) => {
      const tag = (v && v.releaseTag) ? v.releaseTag : 'Development/Test';
      document.title = 'Macroverse 42 - The Wired Atelier [' + tag + ']';
      const tagEl = el('splashVersion');
...
export async function applyServerConfig(): Promise<void> {
  try {
    ...
    if (!r.ok) return;
    const cfg = await r.json() as { readonly?: boolean; demo?: boolean };
    const banner = document.getElementById('demoBanner');
    if (banner && (cfg.readonly || cfg.demo)) {
...
function initColResizer(): void {
  ...
  res.onmousedown = () => { down = true; document.body.style.cursor = 'col-resize'; };
...
function showHelpModal(): void {
  ...
  overlay.style.cssText = 'position:fixed;inset:0;z-index:10003;background:rgba(0,0,0,0.8);display:flex;align-items:center;justify-content:center;';
  overlay.onclick = (e) => { if (e.target === overlay) overlay.remove(); };

  const box = document.createElement('div');
  box.style.cssText = 'background:var(--amiga-panel);border:2px solid var(--amiga-copper);padding:0;max-width:700px;width:90vw;max-height:85vh;display:flex;flex-direction:column;font-family:inherit;';
  box.onclick = (e) => e.stopPropagation();

  const header = document.createElement('div');
  header.style.cssText = 'padding:10px 16px;background:repeating-linear-gradient(90deg,var(--amiga-surface) 0px,var(--amiga-surface) 2px,var(--amiga-panel) 2px,var(--amiga-panel) 4px);border-bottom:2px solid var(--bevel-dark);display:flex;justify-content:space-between;align-items:center;flex-shrink:0;';
  header.innerHTML = '<span style="color:var(--amiga-copper);font-size:14px;text-transform:uppercase;letter-spacing:0.15em;font-weight:bold">Macroverse Help</span>';
  const closeBtn = document.createElement('button');
  closeBtn.type = 'button';
  closeBtn.textContent = 'X';
  closeBtn.style.cssText = 'background:transparent;border:1px solid var(--bevel-dark);color:var(--amiga-copper);padding:2px 8px;cursor:pointer;font-size:12px;';
  closeBtn.onclick = () => overlay.remove();
  header.appendChild(closeBtn);

  const content = document.createElement('div');
  content.style.cssText = 'padding:16px;overflow-y:auto;flex:1;min-height:0;font-size:12px;color:var(--crt-fg);line-height:1.6;';
  content.innerHTML = `
...
<div style="margin-bottom:16px">
  ...
  <div style="padding-left:8px;border-left:2px solid var(--crt-dim);margin-bottom:8px">
    <b>MacroCam:</b> MJPEG stream at <code>/api/output/macrocam/stream</code> for OBS, virtual webcam.<br>
    <b>Spout/NDI:</b> Right-click the preview canvas. Platform support varies (e.g. Spout on Windows). Requires wire-output binary.
  </div>
</div>
...
<div style="margin-bottom:16px">
  ...
  <div style="padding-left:8px;border-left:2px solid var(--crt-dim)">
    ...
    <b>Right-click preview:</b> Output options (Spout/NDI, etc.)<br>
...
function initIndexToolsToggle(): void {
  ...
  btn.addEventListener('click', () => {
    ...
    btn.textContent = expanded ? 'Hide tools \u2191' : 'More tools \u2193';
  });
}
...
function initToolbarMore(): void {
  ...
  if (window.matchMedia('(max-width: 640px)').matches && codeToolbar) {
