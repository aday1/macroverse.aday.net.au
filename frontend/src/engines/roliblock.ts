 ...
 * With 2 devices in VJ mode, device 1 → Deck A, device 2 → Deck B.
 */
...
export type LedDisplayMode = 'independent' | 'stretched' | 'linked';
...
export class RoliblockDevice {
  ...
  deckAssignment: DeckAssignment = 'auto';
...
export function sendStretchedLed(devices: RoliblockDevice[], canvas: HTMLCanvasElement): void {
  ...
  for (let d = 0; d < 2 && d < devices.length; d++) {
    ...
    const now = Date.now();
    if (now - dev.lastLedSend < LED_SEND_INTERVAL_MS) continue;
    dev.lastLedSend = now;
...
export const roliblockManager = {
  ...
  ledDisplayMode: 'independent' as LedDisplayMode,
  ...
  autoDetectDevices(): void {
    ...
    // Auto-assign decks: first device → deckA, second → deckB
    const devs = this.getDevices();
    if (devs.length >= 2) {
      if (devs[0].deckAssignment === 'auto') devs[0].deckAssignment = 'deckA';
      if (devs[1].deckAssignment === 'auto') devs[1].deckAssignment = 'deckB';
    }
    this.saveAll();
  },
  ...
  loadAll(): void {
    try {
      ...
      if (data.ledDisplayMode) this.ledDisplayMode = data.ledDisplayMode;
