...
const FFT_AUTO_SELECT_KEY = 'macroverse-fft-auto-select';

export function isFftAutoSelectEnabled(): boolean {
  try {
    const s = localStorage.getItem(FFT_AUTO_SELECT_KEY);
...
export function setFftAutoSelectEnabled(enabled: boolean): void {
  try {
    localStorage.setItem(FFT_AUTO_SELECT_KEY, enabled ? '1' : '0');
  } catch (_) {}
}

export function autoAssignFftBandMap(
  map: Record<number, string>,
  paramIds: readonly string[]
): void {
  for (let b = 0; b < FFT_BAND_COUNT; b++) {
    if (b < paramIds.length) map[b] = paramIds[b];
    else delete map[b];
  }
}
